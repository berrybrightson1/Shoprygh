...
export default function AdminRightSidebar({ user }: { user: any }) {
    ...
    const { isRightSidebarOpen, closeRightSidebar } = useUIStore();
    ...
    return (
        <>
            ...
            {isRightSidebarOpen && (
                <div
                    className="fixed inset-0 bg-gray-900/20 backdrop-blur-sm z-[60] xl:hidden"
                    onClick={closeRightSidebar}
                />
            )}

            <aside className={`
                fixed inset-y-0 right-0 z-[70] w-full max-w-[380px] bg-white/90 backdrop-blur-xl border-l border-gray-100 flex flex-col transition-transform duration-300 ease-in-out
                xl:relative xl:transform-none xl:flex xl:w-[420px] xl:max-w-none xl:bg-gray-50/50 xl:z-20
